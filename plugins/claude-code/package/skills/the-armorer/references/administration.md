...
For initial readiness, use `connection_assess` with `harness: "claude-code"` and the
selected paths. Human equivalent: `connection assess --harness claude-code`. It needs
...
The human command is `connection armorer --harness claude-code`; `connection doctor --harness claude-code` is its compatible
alias. The stable typed name remains `claude_code_connection_doctor`. Supply the selected
`state_dir`, `native_home` and `native_binary`; add CLI `--read-only`. Read both
successful reports and an error's `claude_code_connection_report` rather than discarding a
...
For requested repair, use `claude_code_connection_repair_plan` with the retained connection
root, then `claude_code_connection_apply` with `{ "plan": PLAN, "sessions_stopped": true }` once every Claude Code session using that profile has exited. An absent/edited receipt,
...
Use `claude_code_connection_plan` with the chosen binding, source executable, native profile,
native executable and installation state. Review returned identities and targets,
then use `claude_code_connection_apply` with `{ "plan": PLAN }` for an authorized initial installation. Replacement requires `sessions_stopped: true` after every session using that profile exits; omission defers it. Preserve existing
...
new runtime's `claude_code_connection_plan`/`claude_code_connection_apply` so its own embedded plugin is
...
Native replacement can remove cached dependencies still used by existing Claude Code
...
Where advertised by the selected runtime's operation catalog, `claude_code_connection_apply`
