...
Managed connections include a local `references/connection.json` next to the
bundled reference. If present, read it: version 1 provides `runtime`, `binding`,
...
An explicit user-selected target takes precedence. Nonempty `MANDALORE_BIN` and
`MANDALORE_BINDING` overrides may select a different runtime or bank; inspect
only those values, not the whole environment, and resolve a mismatch before
changing anything. Runtime overrides must be absolute executable paths. Do not
