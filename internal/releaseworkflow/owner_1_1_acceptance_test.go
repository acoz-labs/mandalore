...
func TestRoadmapOwnerAcceptanceScope(t *testing.T) {
	...
	for _, pair := range []struct{ source, artifact string }{{ownerCandidate, ownerArtifact}, {armorerCandidate, armorerArtifact}, {roadmapCandidate, roadmapArtifact}} {
		for issue := 0; issue <= 100; issue++ {
	...
	for _, scenario := range []string{"old-source", "old-artifact", "armorer-source", "armorer-artifact", "earlier-1.1", "wrong-artifact", "wrong-source", "wrong-repo", "leading-zero", "negative-issue", "spaced-issue", "no-owner", "wrong-owner", "no-reviewer", "no-confirmation", "false-confirmation", "uppercase-confirmation", "missing-arg", "extra-arg", "case-insensitive"} {
		t.Run(scenario, func(t *testing.T) {
			args := []string{"acoz-labs/mandalore", roadmapCandidate, "93", roadmapArtifact, "example-owner"}
