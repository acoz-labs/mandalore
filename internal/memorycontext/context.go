...
// RecallFreshness makes ordinary freshness an explicit agent choice without
// requiring knowledge of an invisible remote update. It never executes a sync.
const RecallFreshness = " At the start of ordinary memory work, call memory_sync with timeout_seconds: 3 once when the current task and connection permit synchronization, before treating local evidence as current. Do not wait for a cross-machine cue. Do not repeat for every lookup; retry only after a relevant change. If synchronization is prohibited or unavailable, use local evidence and state its freshness limitation."
