...
var synchronization = []Operation{
	...
	operation("memory_sync", "Checkpoint, fetch, validate, integrate and push the bound signet through its native Git origin. Never force-push. Pending/offline or semantic-conflict status is not complete delivery/agreement. Do not sync a no-save/read-only task.", false, func(ctx context.Context, s *memory.Service, in SyncInput) (signetsync.Status, error) {
