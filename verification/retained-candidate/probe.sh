...
case "$target_os/$target_arch/$(uname -s)/$(uname -m)" in
  linux/arm64/Linux/aarch64|darwin/amd64/Darwin/x86_64) ;;
...
jq -e --arg os "$target_os" --arg arch "$target_arch" '.ok and .result.os==$os and .result.arch==$arch and .result.source_commit=="5b3c7b275bb4bbb058bebdd7153f4b76bbceb59f"' "$results/version.json" >/dev/null
...
jq -n --arg os "$target_os" --arg arch "$target_arch" --arg digest "$expected" '{passed:true,os:$os,arch:$arch,binary_sha256:$digest,source_commit:"5b3c7b275bb4bbb058bebdd7153f4b76bbceb59f",manifest_sha256:"c46743709dcd02b106c4e6d35cf58247486537e0212603eb7e369ecdfa1bd237",native_agent:false,model_used:false,checks:["actual retained binary version","same-record correction/history","MCP stdio and read-only hash window","local Git delivery and fresh-clone recall","owned CLI install and exact launcher identity","synthetic migration and original snapshot","exact historical JSON number","unrelated cwd unchanged"]}' > "$results/summary.json"
